...
apply_production_config(client_app)
client_app.config['SECRET_KEY'] = os.getenv('CLIENT_SECRET_KEY', 'clientsecretkey123')
configure_sqlalchemy(client_app)
client_app.config['SESSION_COOKIE_NAME'] = 'nexus_client_session'
client_app.config['MAX_CONTENT_LENGTH'] = 100 * 1024 * 1024
...
socketio = SocketIO(client_app, **_socketio_kw)

UPLOAD_FOLDER = 'static/uploads/voice'
if not os.path.exists(UPLOAD_FOLDER):
    os.makedirs(UPLOAD_FOLDER, exist_ok=True)

db.init_app(client_app)
with client_app.app_context():
    run_client_database_bootstrap(client_app)

login_manager = LoginManager(client_app)
...
attach_realtime(socketio, client_app, FlaskSessionBackend())
...
@client_app.route('/api/switch_ecosystem', methods=['POST'])
@login_required
...
@client_app.route('/api/get_team_members')
@login_required
...
@client_app.route("/api/socket_token", methods=["GET"])
@login_required
def api_socket_token():
    return jsonify(token=create_socket_token(current_user.id))


@client_app.route('/api/get_online_users')
@login_required
...
@client_app.route('/api/get_cross_ecosystem_dms')
@login_required
...
@client_app.route('/api/get_dm_permissions')
@login_required
...
@client_app.route('/api/update_dm_permission', methods=['POST'])
@login_required
...
@client_app.route('/api/toggle_user_restriction', methods=['POST'])
@login_required
...
@client_app.route('/api/set_dm_allowlist_only', methods=['POST'])
@login_required
...
@client_app.route('/api/send_message', methods=['POST'])
@login_required
def send_message():
    data = request.json
...
@client_app.route('/api/get_messages')
@login_required
def get_messages():
...
@client_app.route('/api/get_unread_counts')
@login_required
...
@client_app.route('/api/get_channel_unread')
@login_required
...
@client_app.route('/api/get_activity')
@login_required
def get_activity():
...
@client_app.route('/api/mark_activity_read', methods=['POST'])
@login_required
...
# --- CHANNEL API ---
@client_app.route('/api/get_channels')
@login_required
def get_channels():
...
@client_app.route('/api/get_groups')
@login_required
def get_groups():
...
@client_app.route('/api/update_channel_metadata', methods=['POST'])
@login_required
...
@client_app.route('/api/update_channel_visibility', methods=['POST'])
@login_required
...
@client_app.route('/api/create_channel', methods=['POST'])
@login_required
def create_channel():
...
@client_app.route('/api/create_private_group', methods=['POST'])
@login_required
...
@client_app.route('/api/leave_group', methods=['POST'])
@login_required
def leave_group():
    data = request.json
...
@client_app.route('/api/disband_group', methods=['POST'])
@login_required
def disband_group():
    if not is_channel_manager(current_user):
        return jsonify({'error': 'Unauthorized'}), 403
...
@client_app.route('/api/get_group_details/<int:group_id>')
@login_required
...
@client_app.route('/api/add_group_member', methods=['POST'])
@login_required
...
@client_app.route('/api/remove_group_member', methods=['POST'])
@login_required
...
@client_app.route('/api/get_channel_post_permissions')
@login_required
...
@client_app.route('/api/update_channel_post_permissions', methods=['POST'])
@login_required
...
@client_app.route('/api/search_ecosystem_members')
@login_required
...
@client_app.route('/api/get_ecosystem_members')
@login_required
...
@client_app.route('/api/get_workspace_members')
@login_required
...
@client_app.route('/api/update_group_visibility', methods=['POST'])
@login_required
...
@client_app.route('/api/create_ecosystem', methods=['POST'])
@login_required
...
@client_app.route('/api/toggle_workspace_privacy', methods=['POST'])
@login_required
...
@client_app.route('/api/get_public_ecosystems')
@login_required
...
@client_app.route('/api/join_ecosystem', methods=['POST'])
@login_required
def join_ecosystem():
    data = request.json
    ws_id = data.get('workspace_id')
...
@client_app.route('/api/share_ecosystem_with_team', methods=['POST'])
@login_required
def share_ecosystem_with_team():
    ...
    db.session.commit()
    return jsonify({'success': True})
@client_app.route('/api/get_team_members_for_sharing')
@login_required
...
@client_app.route('/api/update_ecosystem_access', methods=['POST'])
@login_required
...
@client_app.route('/api/update_ecosystem_identity', methods=['POST'])
@login_required
...
@client_app.route('/api/delete_ecosystem', methods=['POST'])
@login_required
...
@client_app.route('/api/get_ecosystems')
@login_required
def get_ecosystems():
...
@client_app.route('/api/switch_workspace', methods=['POST'])
@login_required
...
@client_app.route('/api/upload_voice', methods=['POST'])
@login_required
def upload_voice():
...
@client_app.route('/api/upload_image', methods=['POST'])
@login_required
def upload_image():
...
@client_app.route('/api/upload_attachment', methods=['POST'])
@login_required
...
@client_app.route('/api/update_user_profile', methods=['POST'])
@login_required
...
if __name__ == '__main__':
    ...
    socketio.run(client_app, host='0.0.0.0', port=5001, debug=True)
