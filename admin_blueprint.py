"""Admin routes (blueprint) for unified Nexus app."""
import os
import uuid
import datetime
import json
...
@admin_bp.route('/admin/update_team_deployment/<int:team_id>', methods=['POST'])
@login_required
def update_team_deployment(team_id):
    if current_user.role != 'superadmin':
        flash('Access denied.', 'error')
        return redirect(url_for('admin.admin_dashboard')) # Changed 'dashboard' to 'admin_dashboard' for consistency
...
@admin_bp.route('/admin/toggle_ecosystem_visibility/<int:ws_id>', methods=['POST'])
@login_required
def toggle_ecosystem_visibility(ws_id):
    ...
    return redirect(url_for('admin.admin_dashboard') + '?section=ecosystems')

@admin_bp.route('/admin/toggle_group_creation/<int:ws_id>', methods=['POST'])
@login_required
...
@admin_bp.route('/admin/api/update_workspace', methods=['POST'])
@login_required
def api_update_workspace():
    ...
    if request.is_json:
        ...
        db.session.commit()
        log_action(f"Admin updated workspace configuration: {ws.name}")
        return jsonify({"success": True, "name": ws.name, "logo_url": ws.logo_url})
    ...
    if logo_file and logo_file.filename:
        upload_path = os.path.join(current_app.root_path, 'static', 'img', 'workspace_logos')
...
@admin_bp.route('/admin/personal/update_password', methods=['POST'])
@login_required
...
@admin_bp.route('/admin/update_password/<int:user_id>', methods=['POST'])
@login_required
...
@admin_bp.route('/admin/update_designation/<int:user_id>', methods=['POST'])
@login_required
...
@admin_bp.route('/admin/api/add_workspace', methods=['POST'])
@login_required
def add_workspace():
    if not is_superadmin(current_user):
...
@admin_bp.route('/admin/api/get_team_channels', methods=['POST'])
@login_required
...
@admin_bp.route('/admin/api/analyze_teams', methods=['POST'])
@login_required
def analyze_teams():
    if current_user.role != 'admin':
        return {"error": "Unauthorized"}, 403
